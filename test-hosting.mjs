...
s=createGame(d);s.hosting=plan({hpBelow:30});s.hp=25;step=hostedStep(s);check(step.action==='rest'&&step.state.hp>30,'health below 30 schedules one rest that restores above 30');step=hostedStep(step.state);check(step.action==='work','health above 30 immediately resumes work when no recovery target is selected');
