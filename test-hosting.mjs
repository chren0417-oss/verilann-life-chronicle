...
s=createGame(d);s.hosting=plan({staminaBelow:35,staminaRecoverTo:95});s.stamina=20;step=hostedStep(s);check(step.action==='rest'&&step.state.hosting.resting,'hosting remembers an unmet recovery target');step=hostedStep(step.state);check(step.action==='rest'&&step.state.stamina>=95,'hosting rests again only while the recovery target remains unmet');
