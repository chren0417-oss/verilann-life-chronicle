...
const {validateDecision,decisionSchema,planningEconomics,makeGoal}=await importTS('lib/goal-engine.ts');
...
const ongoing=structuredClone(state);ongoing.aiGoal=makeGoal(ongoing,'攒钱',decision,null);ongoing.day=ongoing.aiGoal.deadlineDay-1;
const expiredStep=await planWithAI(env,ongoing,'攒钱',null,async()=>completion());
check(expiredStep.status==='blocked'&&expiredStep.next===null,'continuation respects original deadline and validates fallback too');
const shifted={...decision,interpretation:'换成练剑',milestones:[{label:'剑术',kind:'skill',key:'剑术',target:80}]};
ongoing.day=ongoing.aiGoal.createdDay;
const preserved=await planWithAI(env,ongoing,'攒钱',null,async()=>completion(JSON.stringify(shifted)));
check(JSON.stringify(preserved.milestones)===JSON.stringify(decision.milestones)&&preserved.interpretation===decision.interpretation,'provider cannot move existing goalposts');
...
let repairCalls=0;
const repaired=await planWithAI(env,state,'x',null,async(_url,init)=>{repairCalls++;const outgoing=JSON.parse(init.body);if(repairCalls===1)return completion(JSON.stringify({...decision,priorities:[]}));check(outgoing.messages[0].content.includes('修正'), 'invalid JSON gets one compact repair request');return completion(JSON.stringify(decision))});
