...
let q=createGame(d);q=perform(q,'quest').state;const count=q.quests.length;check(!!perform(q,'quest').error&&q.quests.length===count,'one active quest');check(!!perform(q,'deliver').error,'deliver needs active skill quest');q.quests[0]={...q.quests[0],kind:'委托',skill:'锻造',need:40,due:q.day+45,status:'进行中'};q.skills['锻造']=39;check(!!perform(q,'deliver').error,'quest skill prerequisite');q.skills['锻造']=40;q=perform(q,'deliver').state;check(q.quests[0].status==='已完成','quest completes');const bc=perform(createGame(d),'quest').state;bc.jobsRank=[3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,100];bc.job=16;const bq=bc.quests[0];if(bq.kind==='战斗'||bq.kind==='冒险'){check(!!perform(bc,'battle-quest',String(bq.id)).error===false,'battle quest accepts')}else{check(!!perform(bc,'battle-quest',String(bq.id)).error===true,'non-battle quest rejects battle')}
